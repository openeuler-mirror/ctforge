 ...
 */
int sm2_sign(const char *msg, size_t msg_len,
		...
		unsigned char **sig, size_t *sig_len)
{
	...
	if (EVP_DigestSign(md_ctx, NULL, sig_len, (const unsigned char *)msg, msg_len) <= 0) {
		goto cleanup;
	}
	...
	*sig = OPENSSL_malloc(*sig_len);
	...
	if (EVP_DigestSign(md_ctx, *sig, sig_len, (const unsigned char *)msg, msg_len) <= 0) {
		OPENSSL_free(*sig);
		*sig = NULL;
		ret = -7;
		goto cleanup;
 ...
 */
int sm2_verify(const char *msg, size_t msg_len,
		...
		const unsigned char *sig, size_t sig_len)
{
	...
	if (EVP_DigestVerify(md_ctx, sig, sig_len, (const unsigned char *)msg, msg_len) == 1) {
 ...
 * Returns 0 on success, non-zero on error.
 */
int generate_sm2_signature_header(const char *msg,
		const unsigned char *sig, size_t sig_len,
		const char *pubkey_pem_path,
		const char *header_path)
{
	...
	size_t pubkey_len = 0;
	if (!EVP_PKEY_get_octet_string_param(pub_pkey, OSSL_PKEY_PARAM_PUB_KEY, NULL, 0, &pubkey_len) ||
			pubkey_len != 65) {
	...
	if (!EVP_PKEY_get_octet_string_param(pub_pkey, OSSL_PKEY_PARAM_PUB_KEY,
				pubkey_raw, sizeof(pubkey_raw), &pubkey_len)) {
		EVP_PKEY_free(pub_pkey);
		return -5;
	}
	EVP_PKEY_free(pub_pkey);
...
int main(void)
{
	...
	// Generate header
	rc = generate_sm2_signature_header(msg, sig, sig_len,
			"sm2_public_key.pem",
			"sm2_signature.h");
	if (rc != 0) {
